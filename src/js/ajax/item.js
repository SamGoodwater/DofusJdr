class Item {
    ...
    static open(uniqid)
    {
        var URL = 'index.php?c=item&a=getFromUniqid';
        ...
        $.post(URL,
            ...
            function(data, status)
            {
                if(data['return'] == 'success'){
                    ...
                    MsgAlert("Impossible de récupérer l'équipement", 'Erreur : ' + data.error, "danger" , 4000);
                }
            },
            "json"
        ); 
    ...
    static add(){
        ...
        $.post(URL,
            ...
            function(data, status)
            {
                ...
                if(data['return'] == 'success'){
                    MsgAlert("Ajout de l'équipement", 'L\'équipement ' + name + ' a bien été ajouté.', "success" , 3000);
    ...
    static update(uniqid, input, type, value_type = IS_INPUT, fct = ""){
        var URL = 'index.php?c=item&a=update';
        var value = 0;
    ...
    static remove(uniqid){
        var URL = 'index.php?c=item&a=remove';
    
        if (confirm("Etes vous sûr de vouloir supprimer définitivement cet item ?")) {
            $.post(URL,
                {
                    uniqid:uniqid
                },
                function(data, status)
                {
                    ...
                    if(data['return'] == 'success'){
                        MsgAlert("Suppression de l'item", "L'item a bien été supprimé.", "success" , 3000);
                        $('#table').bootstrapTable('refresh');
                    } else {
                        $('#display_error').text(data['error']);
    ...
    static updateDisplayRow(uniqid) {
        ...
        var URL = 'index.php?c=item&a=getArrayFromUniqid';
        $.post(URL,
            {
                uniqid:uniqid
            },
            function(data, status)
            {
                if(data.return == 'success'){
                    ...
                    $("#table").bootstrapTable('updateCell', {index: indexCell, field: 'actif', value: data.value.actif});
                    $("#table").bootstrapTable('updateCell', {index: indexCell, field: 'twohands', value: data.value.twohands});
                    $("#table").bootstrapTable('updateCell', {index: indexCell, field: 'pa', value: data.value.pa});
                    $("#table").bootstrapTable('updateCell', {index: indexCell, field: 'po', value: data.value.po});
