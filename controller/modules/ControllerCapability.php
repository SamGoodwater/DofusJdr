    ...
    if(!$currentUser->getRight('capability', User::RIGHT_READ)){
      ...
      foreach ($objs as $obj) {
        ...
        if($currentUser->getRight('capability', User::RIGHT_WRITE)){
          $edit = "<a id='{$obj->getUniqid()}' class='text-main-d-2 text-main-l-3-hover' onclick=\"Capability.open('{$obj->getUniqid()}', Controller.DISPLAY_EDITABLE)\"><i class='far fa-edit'></i></a>";
        }
        ...
        $json[] = array(
          ...
          'resume' => $resume,
          'edit' => $edit,
          'pdf' => "<a data-bs-toggle='tooltip' data-bs-placement='top' title='Générer un pdf' class='text-red-d-2 text-red-l-3-hover' target='_blank' href='index.php?c=capability&a=getPdf&uniqids=".$obj->getUniqid()."'><i class='fas fa-file-pdf'></i></a>",
        ...
        if(!empty($objects)){
            ...
            foreach ($objects as $object) {
                ...
                switch ($action) {
                  ...
                  break;
                  case ControllerSearch::SEARCH_DONE_ADD_CAPABILITY_TO_CLASSE:
                    $click_action = "onclick=\"Classe.update('".$parameter."',{action:'add', uniqid:'".$object->getUniqid()."'},'capability', IS_VALUE);\"";
                  break;
                  case ControllerSearch::SEARCH_DONE_ADD_CAPABILITY_TO_NPC:
