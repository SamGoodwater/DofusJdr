...
class ControllerTools extends Controller
{
																...
																case self::TEMPLATE_ITEMS:
																	...
																	if($extract['state']){
																		if (
																			...
																			ob_start(); ?>
																			<div class="m-4">
																				<p><?= $extract['obj']->getName() ?> - Niv. <?= $extract['obj']->getLevel() ?> <img width='30px' src='<?= $extract['obj']->getFile(name_file : 'logo', style : new Style(["display" => Content::FORMAT_BRUT]), getThumbnail: true) ?>'></p>
	...
	// Extraction des Items, Consommables et Ressources
		private function extractItem($data, $dofus_version, $write = false): array {
			...
			// IMAGES
				$url_img = $data['imgset'][3]['url'];
				$url_img_thumbnail = $data['imgset'][1]['url'];
									...
									foreach ($recipe as $rec) {
										...
										if($write == true && $rec['action'] == "add"){
											FileManager::add(
												...
												rename: $rec['ressource']->getUniqid() . ".png", 
												compress: false, 
											...
											FileManager::add(
												...
												dir_dest: Ressource::FILES['logo']['dir'], 
												rename: $rec['ressource']->getUniqid() . "_thumb" . ".png", 
												compress: false, 
												set_format_webp: false, 
												resize: false, 
												thumbnail: false
						...
						if ($object_type != "Inconnu" && $write == true) {
							if ($manager->add($obj)) {
								...
								FileManager::add(
									...
									dir_dest: $path_img, 
									rename: $obj->getUniqid() . "_thumb" . ".png", 
									compress: false, 
									set_format_webp: false, 
									resize: false, 
									thumbnail: false
	...
	// Extractions secondaires
		...
		private function extractRessource($data, $dofus_version){
			...
			if($manager->existsDofusdb_id($data['id'])) {
				...
				$img = [
					'path' => $data['imgset'][3]['url'],
					'thumbnail' => $data['imgset'][1]['url']
				];
			}
