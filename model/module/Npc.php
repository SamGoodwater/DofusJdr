...
class Npc extends Content
{
    ...
    const DIR_IMG = "medias/npc/";
    const PARH_DEFAULT = "medias/npc/default.jpg";
    ...
    //♥♥♥♥♥♥♥♥♥♥♥♥♥♥ GETTERS ♥♥♥♥♥♥♥♥♥♥♥♥♥♥
        ...
        public function getPath_img(int $format = Content::FORMAT_BRUT, $css = ""){
            $view = new View();
            $file = new File(Npc::PARH_DEFAULT);
            $classe = $this->getClasse(Content::FORMAT_OBJECT);

            if(file_exists(Npc::DIR_IMG.$this->getUniqid().".png")){
                $file = new File(Npc::DIR_IMG.$this->getUniqid().".png");
            } elseif(file_exists($classe->getPath_img(Content::FORMAT_BRUT))) {
                $file = new File($classe->getPath_img(Content::FORMAT_BRUT));
            } else {
                $file = new File(Npc::PARH_DEFAULT);
            }
            ...
            switch ($format) {
                ...
                case  Content::DISPLAY_EDITABLE:
                    ob_start();
                    $file->getVisual(Content::FORMAT_VIEW);
                    $view->dispatch(
                        template_name : "input/file",
                        data : [
                            "url" => "index.php?c=npc&a=upload",
                            "uniqid" => $this->getUniqid(),
                            "label" => "Image du ou de la PNJ",
                            "view_img_path" => $file->getPath(),
                            "extention_available" => FileManager::getListeExtention(FileManager::FORMAT_IMG)
                        ], 
                        write: true);
                        ?><div><a onclick="File.remove('<?=$file->getPath();?>');" class="btn-sm btn-text-main">Supprimer l'image</a></div><?php
                    return ob_get_clean();
                break;
                default:
                    return $file->getVisual();
                break;
            }
        }
    ...
    //♥♥♥♥♥♥♥♥♥♥♥♥♥♥ SETTERS ♥♥♥♥♥♥♥♥♥♥♥♥♥♥
        ...
        public function setOther_item($data){
