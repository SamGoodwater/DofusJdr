...
// Link Consumable
    public function getLinkConsumable(Shop $shop){
        $req = $this->_bdd->prepare('SELECT *, link_shop_consumable.price AS price_link FROM link_shop_consumable INNER JOIN consumable ON link_shop_consumable.id_consumable = consumable.id WHERE link_shop_consumable.id_shop = ? ORDER BY link_shop_consumable.price');
    ...
    public function getLinkConsumableFromConsumable(Shop $shop , Consumable $consumable){
        $req = $this->_bdd->prepare('SELECT *, link_shop_consumable.price AS price_link FROM link_shop_consumable INNER JOIN consumable ON link_shop_consumable.id_consumable = consumable.id WHERE link_shop_consumable.id_shop = ? AND link_shop_consumable.id_consumable = ? ORDER BY link_shop_consumable.price');
...
// Link Item
    public function getLinkItem(Shop $shop){
        $req = $this->_bdd->prepare('SELECT *, link_shop_item.price AS price_link FROM link_shop_item INNER JOIN item ON link_shop_item.id_item = item.id WHERE link_shop_item.id_shop = ? ORDER BY link_shop_item.price');
        $req->execute(array($shop->getId()));
        $ret = $req->fetchAll(PDO::FETCH_ASSOC);
        if(empty($ret)){return "";}
        $return = array();
    ...
    public function getLinkItemFromItem(Shop $shop , Item $item){
        $req = $this->_bdd->prepare('SELECT *, link_shop_item.price AS price_link FROM link_shop_item INNER JOIN item ON link_shop_item.id_item = item.id WHERE link_shop_item.id_shop = ? AND link_shop_item.id_item = ? ORDER BY link_shop_item.price');
