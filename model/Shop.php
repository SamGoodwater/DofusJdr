    ...
    //♥♥♥♥♥♥♥♥♥♥♥♥♥♥ GETTERS ♥♥♥♥♥♥♥♥♥♥♥♥♥♥
        ...
        public function getId_seller(int $format = Content::FORMAT_BRUT){
            ...
            switch ($format) {
                case Content::FORMAT_MODIFY:
                    ob_start(); ?>
                        <p>Marchant de l'hôtel de vente (PNJ)</p>
                        <div class="dropdown">
                ...
                case Content::FORMAT_RESUME:
                    if(isset($npc)){
                        return $npc->getVisual(Content::FORMAT_RESUME);
                    } else {
                        return "";
                    }
                ...
                case Content::FORMAT_BADGE:
                    if(isset($npc)){
                        return "<span class='badge text-black back-white border-1 border-solid border-green-d-2' data-bs-toggle='tooltip' data-bs-placement='bottom' title=\"Marchant gérant l'hôtel de vente\">".$npc->getName()." - <i class='text-grey-d-2'>".$npc->getClasse(Content::FORMAT_OBJECT)->getName()."</i></span>";
                    } else {
                        return "";
                    }
                            ...
                            if(!empty($links)){
                                foreach ($links as $link) { ?>
                                    <div style="width: 15rem;position:relative;">
                                        <div class="card border border-grey card-hover-linked" style="width: 15rem;">
                                            <div class="text-center" style="position:absolute;top:5px;right:7px;z-index:9;height:30px;width:30px;">
                                                <a data-bs-toggle='tooltip' data-bs-placement='bottom' title="Dissocier ce consommable de cet hôtel de vente" class="<?=View::getCss(View::TYPE_BTN_UNDERLINE, "red")?>" onclick="if (confirm('Etes vous sûr dissocier le consommable de cet hôtel de vente ?')){Shop.update('<?=$this->getUniqid()?>',{action:'remove', uniqid:'<?=$link['obj']->getUniqid()?>'},'consumable', IS_VALUE);}"><i class="fas fa-times"></i></a>
                                            </div>
                                            <a class="text-left" style="position:relative;top:5px;left:5px;" href="<?=$link["obj"]->getPath_img()?>" download="<?=$link["obj"]->getName().'.'.substr(strrchr($link["obj"]->getPath_img(),'.'),1);?>"><i class="fas fa-download text-main-d-3 text-main-d-1-hover"></i></a>        
                                            <?=$link["obj"]->getPath_img(Content::FORMAT_FANCY, "img-back-120H-allL")?>
                                                ...
                                                <div class="row justify-content-between">
                                                    ...
                                                    <p class="d-flex flex-row justify-content-around mt-1 badge back-white border border-2 border-kamas-d-4">
                                                        <?php if(!empty($link['price'])){ ?>
                                                            <span class='text-kamas-d-4 size-1-2' data-bs-toggle='tooltip' data-bs-placement='bottom' title="Prix du consommable"><?=$link["price"]?> <img class='icon' src='medias/icons/kamas.png'></span>     
                                                        <?php } ?>
                                            ...
                                            <div class="card-body" id="modifyConsumable<?=$link['obj']->getUniqid()?>">
                                                ...
                                                <div class="form-floating mb-3">
                                                    <input value="<?=$link['quantity']?>" id="quantityInput<?=$link['obj']->getUniqid()?>" onchange="Shop.update('<?=$this->getUniqid()?>',{action:'update', uniqid:'<?=$link['obj']->getUniqid()?>', quantity:$(this).val()},'consumable', IS_VALUE);" type="text" class="form-control" placeholder="Quantité">
                                                    <label for="quantityInput<?=$link['obj']->getUniqid()?>">Quantité</label>
                                                </div>
                                                <div class="form-floating mb-3">
                                                    <input value="<?=$link['price']?>" id="priceInput<?=$link['obj']->getUniqid()?>" onchange="Shop.update('<?=$this->getUniqid()?>',{action:'update', uniqid:'<?=$link['obj']->getUniqid()?>', price:$(this).val()},'consumable', IS_VALUE);" type="text" class="form-control" placeholder="Prix">
                                                    <label for="priceInput<?=$link['obj']->getUniqid()?>">Prix</label>
                                                </div>
                                                <div class="form-floating mb-3">
                                                    <input value="<?=$link['comment']?>" id="commentInput<?=$link['obj']->getUniqid()?>" onchange="Shop.update('<?=$this->getUniqid()?>',{action:'update', uniqid:'<?=$link['obj']->getUniqid()?>', comment:$(this).val()},'consumable', IS_VALUE);" type="text" class="form-control" placeholder="Commentaire">
                                                    <label for="commentInput<?=$link['obj']->getUniqid()?>">Commentaire</label>
                                                </div>
                                                <div>
                                                    <a class="btn btn-sm btn-back-secondary" onclick="Consumable.open('<?=$link['obj']->getUniqid()?>');">Modifier le consommable</a>
                                                </div>
                                                <div class="nav-item-divider back-main-d-1"></div>
                                    ...
                                    <div class="m-2" style="width: 15rem;position:relative;">
                                        <div class="card border border-grey card-hover-linked" style="width: 15rem;">
                                            ...
                                            <div class="card-body" id="modifyItem<?=$link['obj']->getUniqid()?>">
                                                <h5 class="card-title"><?=$link["obj"]->getName()?></h5>
                                                <div class="form-floating mb-3">
                                                    <input value="<?=$link['quantity']?>" id="quantityInput<?=$link['obj']->getUniqid()?>" onchange="Shop.update('<?=$this->getUniqid()?>',{action:'update', uniqid:'<?=$link['obj']->getUniqid()?>', quantity:$(this).val()},'item', IS_VALUE);" type="text" class="form-control" placeholder="Quantité">
                                                    <label for="quantityInput<?=$link['obj']->getUniqid()?>">Quantité</label>
                                                </div>
                                                <div class="form-floating mb-3">
                                                    <input value="<?=$link['price']?>" id="priceInput<?=$link['obj']->getUniqid()?>" onchange="Shop.update('<?=$this->getUniqid()?>',{action:'update', uniqid:'<?=$link['obj']->getUniqid()?>', price:$(this).val()},'item', IS_VALUE);" type="text" class="form-control" placeholder="Prix">
                                                    <label for="priceInput<?=$link['obj']->getUniqid()?>">Prix</label>
                                                </div>
                                                <div class="form-floating mb-3">
                                                    <input value="<?=$link['comment']?>" id="commentInput<?=$link['obj']->getUniqid()?>" onchange="Shop.update('<?=$this->getUniqid()?>',{action:'update', uniqid:'<?=$link['obj']->getUniqid()?>', comment:$(this).val()},'item', IS_VALUE);" type="text" class="form-control" placeholder="Commentaire">
                                                    <label for="commentInput<?=$link['obj']->getUniqid()?>">Commentaire</label>
                                                </div>
                                                <div>
        ...
        public function getVisual(int $format = Content::FORMAT_BRUT){
            ...
            switch ($format) {
                case Content::FORMAT_MODIFY:      
                    ob_start(); ?>
                        <div class="card mb-3">
                            <p class='size-0-7 mb-1'>Hôtel de vente N°<?=$this->getId(Content::FORMAT_BADGE);?> | Créé le <?=$this->getTimestamp_add(Content::DATE_FR);?> | Modifié le <?=$this->getTimestamp_updated(Content::DATE_FR);?></p>
                                <div>
                                    <div>
                                        <p><?=$this->getLocation(Content::FORMAT_MODIFY)?></p>
                                        <p><?=$this->getPrice(Content::FORMAT_MODIFY)?></p>
                                        <p><?=$this->getDescription(Content::FORMAT_MODIFY)?></p>
                                    </div>
                                    <div>
                                        <h3 class="text-center">Marchant</h3>
                                        <div class="row justify-content-center">
                                            <?=$this->getId_seller(Content::FORMAT_RESUME)?>
                                        </div>
                                    </div>
                                </div>
                ...
                case  Content::FORMAT_CARD:      
                    ob_start(); ?>
                        <div class="card mb-3">
                            <p class='size-0-7 mb-1'>Hôtel de vente <?=$this->getId(Content::FORMAT_BADGE);?> | Créé le <?=$this->getTimestamp_add(Content::DATE_FR);?> | Modifié le <?=$this->getTimestamp_updated(Content::DATE_FR);?></p>
                                <div class="d-flex flex-row justify-content-around flex-wrap">
                                    <div class="col-6 col-xs-12">
                                        <h1 class="m-2 mx-4"><?=$this->getName()?></h1>
                                    ...
                                    <div class="col-6 col-xs-12">
                                        <h4 class="text-center">Marchant</h4>
                                        <div class="row justify-content-center">
                                            <?=$this->getId_seller(Content::FORMAT_RESUME)?>
                                        </div>
                                    </div>
                                ...
                                <div class="card-body text-center">
                                    <h3>Equipements</h3>
                                    <?=$this->getItem(Content::FORMAT_MODIFY)?>
                                    <h3>Consommables</h3>
                                    <?=$this->getConsumable(Content::FORMAT_MODIFY)?>
                                </div>
                                <p class="text-right font-size-0-8 m-1"><a class='text-red-d-2 text-red-l-3-hover' onclick="Shop.remove('<?=$this->getUniqid()?>')"><i class="fas fa-trash"></i> Supprimer</a></p>
                        </div>
                    <?php return ob_get_clean();
                ...
                case Content::FORMAT_RESUME:
                    ob_start(); ?>
                        <div style="position:relative;">
                            <div ondblclick="Shop.open('<?=$this->getUniqid()?>');" class="card-hover-linked card p-2 m-1 border-secondary-d-2 border-solid border-2" style="width: 300px;" >
                                    ...
                                    <div class="card-body m-1 p-0">
                                        <div class="d-flex flew-row justify-content-between ">
                                            ...
                                            <div class="d-flex flex-row align-content-center">
                                                <div><?=$this->getLocation(Content::FORMAT_ICON)?></div>
                                                <div><?=$this->getPrice(Content::FORMAT_BADGE)?></div>
                                            </div>
                                        </div>
                    ...
                    switch ($data['action']) {
                        ...
                        case "update":
                            if($managerS->existsLinkItem($this, $item)){
                                $link = $managerS->getLinkItem($this, $item);
