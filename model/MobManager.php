...
// Link Spell
    public function getLinkSpell(Mob $mob){
        $req = $this->_bdd->prepare('SELECT * FROM link_mob_spell INNER JOIN mob ON link_mob_spell.id_mob = mob.id WHERE link_mob_spell.id_mob = ?');
        ...
        foreach ($ret as $link) {
            if($manager->existsId($link["id_spell"])){
                $return[] = [
                    "obj" => $manager->getFromId($link["id_spell"])
                ];
            }
        }
        return $return;
